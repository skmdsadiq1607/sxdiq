...
const Contact = () => {
  ...
  return (
    <section id="contact" className="section-padding relative overflow-hidden noise-overlay" style={{ background: 'var(--section-contact-bg)' }}>
      ...
      <div className="absolute bottom-20 right-10 w-[400px] h-[400px] rounded-full opacity-15" style={{ background: `radial-gradient(circle, ${accentColor}10, transparent 70%)` }} />
      <div className="absolute top-20 left-10 w-[300px] h-[300px] rounded-full opacity-10" style={{ background: `radial-gradient(circle, hsl(260 80% 65% / 0.08), transparent 70%)` }} />
      ...
      <div className="container mx-auto max-w-5xl relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="section-heading">
          ...
          <h2 className="text-4xl md:text-5xl font-bold" style={{ background: `linear-gradient(135deg, ${accentColor}, hsl(260 80% 65%))`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Let's Build Something Amazing</h2>
          <p className="text-muted-foreground mt-4 max-w-xl mx-auto text-lg">Have an idea or want to collaborate? I'd love to hear from you!</p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-5 gap-10">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="md:col-span-2 space-y-4">
            ...
            {contactInfo.map((item, i) => (
              <motion.a
                ...
                className="flex items-center gap-4 p-5 rounded-2xl group premium-glass hover:shadow-lg transition-all"
              >
          ...
          <motion.form
            ...
            className="rounded-3xl p-8 space-y-5 md:col-span-3 relative overflow-hidden premium-glass"
            style={{ boxShadow: `0 20px 60px -15px ${accentColor}08` }}
          >
            ...
            <motion.button whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} type="submit" disabled={sending} className="w-full py-4 rounded-2xl text-white font-bold text-base flex items-center justify-center gap-2 transition-all disabled:opacity-70" style={{ background: `linear-gradient(135deg, ${accentColor}, hsl(260 80% 65%))`, boxShadow: `0 8px 32px ${accentColor}30` }}>
              {sending ? (
